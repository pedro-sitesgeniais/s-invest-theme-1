(() => {
  ...
  var require_auth = __commonJS({
    "resources/js/components/auth.js"(exports, module) {
      document.addEventListener("alpine:init", () => {
        ...
        Alpine.data("authPageV2", () => ({
          ...
          showMessage(text, type = "success", duration = 5e3) {
          ...
          handleRegistration() {
            ...
            if (!this.form.terms) {
              this.showMessage("Voc\xEA deve aceitar os termos de uso para continuar", "error");
              return false;
            }
            this.isLoading = true;
          ...
          validateEmail: window.sInvestUtils?.debounce(async function() {
            ...
            try {
              ...
              if (!response.ok) throw new Error("Erro na verifica\xE7\xE3o");
              const data2 = await response.json();
              if (data2.success) {
                this.emailValidation.exists = data2.data.exists;
              }
            } catch (error2) {
          ...
          validateCPF: window.sInvestUtils?.debounce(async function() {
            ...
            try {
              ...
              const data2 = await response.json();
              if (data2.success) {
                this.cpfValidation.exists = data2.data.exists;
              } else {
          ...
          async handlePasswordReset() {
            ...
            if (!this.isValidEmailFormat(this.resetEmail)) {
              this.showMessage("Formato de e-mail inv\xE1lido", "error");
              return;
            }
            this.isLoading = true;
            try {
              ...
              const data2 = await response.json();
              if (data2.success) {
                this.showMessage("E-mail de recupera\xE7\xE3o enviado com sucesso!", "success");
                setTimeout(() => this.setView("login"), 2e3);
              } else {
                this.showMessage(data2.data || "Erro ao enviar e-mail", "error");
              }
            } catch (error2) {
          ...
          async resendConfirmation() {
            if (!this.form.email) {
              this.showMessage("E-mail n\xE3o informado", "error");
              return;
            }
            this.isLoading = true;
            try {
              await new Promise((resolve) => setTimeout(resolve, 1e3));
              this.showMessage("E-mail de confirma\xE7\xE3o reenviado!", "success");
            } catch (error2) {
          ...
          get canSubmitRegistration() {
            const basicValidation = this.form.firstName.trim() && this.form.lastName.trim() && this.form.email.trim() && this.form.password && this.form.passwordConfirm && this.form.cpf.trim() && this.form.phone.trim();
            ...
            return basicValidation && termsAccepted && passwordsMatch && passwordStrong && emailUnique && cpfValid && cpfUnique;
          }
        }));
      });
...
${expression ? 'Expression: "' + expression + '"\n\n' : ""}`, el);
  ...
  module_default.store("sInvestTheme", {
    isLoading: false,
    currentUser: null,
    version: "2.4.0",
    colors: {
  ...
  module_default.data("panelLayout", () => ({
    loading: false,
    ...
    showLoading() {
      this.loading = true;
      module_default.store("sInvestTheme").showLoading();
    },
    hideLoading() {
      this.loading = false;
      module_default.store("sInvestTheme").hideLoading();
    },
    onSectionLoaded() {
      this.hideLoading();
  ...
  module_default.data("sectionLoader", () => ({
    init() {
      this.$nextTick(() => {
  ...
  module_default.data("dashboardData", () => ({
    loading: false,
  ...
  module_default.data("filtrosInvestimentos", () => ({
    filtros: {
      tipo_produto: "",
      imposto: "",
      modalidade: "",
    ...
    temFiltrosAtivos() {
      ...
      return filtrosParaVerificar.some(
        (filtro) => this.filtros[filtro] && this.filtros[filtro] !== ""
      );
    },
    initInfiniteScroll() {
    ...
    async fetchInvestimentos(limpar = false) {
      ...
      try {
        if (!window.investments_ajax) {
          throw new Error("Configura\xE7\xE3o AJAX n\xE3o encontrada");
        }
        ...
        this.erro = error2.message;
        this.exibirErro(error2.message, limpar);
      } finally {
  ...
  module_default.data("painelFiltros", () => ({
    filtros: {
      tipo_produto: "",
      imposto: "",
      modalidade: "",
    ...
    async buscarInvestimentos() {
      ...
      try {
        ...
        if (data2.success) {
          ...
          this.resultados = data2.data.html;
          this.maxPaginas = data2.data.max_pages || 1;
          this.pagina = data2.data.paged || 1;
          ...
          throw new Error(data2.data || "Erro desconhecido no servidor");
        }
      } catch (error2) {
        if (error2.name === "AbortError") {
          return;
        ...
        this.erro = error2.message;
        this.resultados = this.gerarHTMLErro(error2.message);
      } finally {
        this.carregando = false;
        this.requestController = null;
      }
  ...
  module_default.data("meusFiltros", () => ({
    filtros: {
      tipo_produto: "",
      imposto: "",
      modalidade: "",
  ...
  module_default.data("toastNotifications", () => ({
    notifications: [],
    maxNotifications: 5,
  ...
  window.sInvestUtils = {
    ...
    async copyToClipboard(text, showFeedback = true) {
      try {
        ...
        if (showFeedback && typeof module_default !== "undefined") {
          const toastComponent = document.querySelector('[x-data*="toastNotifications"]');
          if (toastComponent && module_default.$data(toastComponent)) {
            module_default.$data(toastComponent).success("Copiado para a \xE1rea de transfer\xEAncia!");
          }
        }
        return true;
      } catch (err) {
        if (showFeedback && typeof module_default !== "undefined") {
          const toastComponent = document.querySelector('[x-data*="toastNotifications"]');
          if (toastComponent && module_default.$data(toastComponent)) {
            module_default.$data(toastComponent).error("Falha ao copiar");
          }
        }
        return false;
      }
    ...
    generateId: (prefix2 = "id") => {
      return `${prefix2}_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    }
  };
  document.addEventListener("alpine:init", () => {
    if ("IntersectionObserver" in window) {
      const imageObserver = new IntersectionObserver((entries, observer2) => {
        entries.forEach((entry) => {
    ...
    document.querySelectorAll(".alert[data-auto-hide]").forEach((alert) => {
      const delay = parseInt(alert.dataset.autoHide) || 5e3;
      setTimeout(() => {
  ...
  if (!window.alpineInitialized) {
    if (typeof module_default !== "undefined") {
